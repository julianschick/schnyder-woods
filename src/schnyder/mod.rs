...
use crate::graph::data_holders::{Edge, NbVertex, Vertex};
...
impl Edge<SchnyderEdgeDirection> {
    fn color(&self, sig: Signum) -> Option<SchnyderColor> {
        return match self.weight {
            ...
            Bicolored(fwd_c, bwd_c) => match sig {
                Forward => Some(fwd_c),
                Backward => Some(bwd_c),
            },
        };
...
impl SchnyderMap {
    pub fn build_simple_stack(
        vertex_count: usize,
        color: SchnyderColor,
    ) -> GraphResult<SchnyderMap> {
        if vertex_count < 3 {
            return GraphErr::new_err(
                "Simple stacks cannot be constructed on less than 3 vertices.",
            );
        }
    ...
    pub fn build_min_edge_wood(
        vertex_count: usize,
        color: SchnyderColor,
    ) -> GraphResult<SchnyderMap> {
        if vertex_count < 3 {
            return GraphErr::new_err(
                "Minimal edge woods cannot be constructed on less than 3 vertices.",
            );
        }
        ...
        let m = (n - 3) / 2;
        let mut upper = Vec::with_capacity(m + 2);
        upper.push(r);
        let mut lower = Vec::with_capacity(m + 2);
        lower.push(b);
    ...
    pub fn build_from_3tree_code(code: &Vec<u8>) -> GraphResult<SchnyderMap> {
        ...
        for c in 0..3 {
            map.set_vertex_weight(
                vertex_indices[trees[c].get_root() as usize],
                Suspension(colors[c]),
            )?;
        }
        ...
        let converted_nb_orders = neighbor_orders
            .iter()
            .enumerate()
            .map(|(i, nb)| {
                (
    ...
    pub fn build_on_triangulation<N, E>(
        ...
        // by convention
        let mut suspension_vertices = smap
            .try_face(outer_face)?
            .angles
            .iter()
        ...
        while frontier.len() > 0 {
            ...
            let (pos, (&left_neighbour, &pivot, &right_neighbour)) = {
                let candidates: Vec<_> = b_singleton
                    .iter()
                    .chain(frontier.iter())
                    .chain(g_singleton.iter())
                    .tuple_windows()
                    .enumerate()
                ...
                match mode {
                    ...
                    SchnyderBuildMode::Random => {
                        candidates[rand.gen_range(0, candidates.len())].clone()
                    }
                }
            };
    ...
    pub fn get_vertex_map(&self, wood2: &SchnyderMap) -> GraphResult<BiMap<VertexI, VertexI>> {
        ...
        if wood1.map.vertex_count() != wood2.map.vertex_count() {
            return GraphErr::new_err(
                "A vertex map can only be found between Schnyder woods of same vertex count",
            );
        }
    ...
    fn get_vertices_in_bfs_order(
        ...
        while let Some(first) = queue.pop_front() {
            ...
            match direction {
                ...
                CCW => queue.extend(
                    self.get_incoming_sector(first, color, true)
                        .into_iter()
                        .rev(),
                ),
            }
    ...
    pub fn compute_3tree_code_with_rotation(
        &self,
        color: SchnyderColor,
        direction: ClockDirection,
    ) -> Vec<u8> {
        ...
        let index_map: HashMap<_, _> = vertices
            .iter()
            .enumerate()
            .map(|(index, v)| (v, index))
            .collect();
    ...
    fn replace_color_nb(
        ...
        color: SchnyderColor,
        sig: Signum,
    ) -> GraphResult<SchnyderEdgeDirection> {
    ...
    fn find_outgoing_nb(&self, vid: VertexI, color: SchnyderColor) -> GraphResult<&NbVertex> {
        let result =
            self.map
                ...
                .neighbors
                .iter()
                .find(|nb| match self.outgoing_color(nb) {
                    Some(cc) if cc == color => true,
                    _ => false,
                });
    ...
    pub fn find_outgoing_endvertex(
        &self,
        vid: VertexI,
        color: SchnyderColor,
    ) -> GraphResult<VertexI> {
    ...
    fn find_outgoing(&self, v: &Vertex<SchnyderVertexType>, c: SchnyderColor) -> Option<usize> {
        v.neighbors
            .iter()
            .position(|nb| match self.outgoing_color(nb) {
                Some(cc) if cc == c => true,
                _ => false,
            })
    }

    fn get_incoming_sector_as_nb(
        &self,
        vid: VertexI,
        color: SchnyderColor,
        with_bicolored: bool,
        ...
        let before_out = v
            .neighbors
            .iter()
            .find(|nb| self.outgoing_color(nb) == Some(color.next()))
            .expect("TODO");
        let after_out = v
            .neighbors
            .iter()
            .find(|nb| self.outgoing_color(nb) == Some(color.prev()))
            .expect("TODO");
    ...
    fn get_incoming_sector_as_edges(
        &self,
        vid: VertexI,
        color: SchnyderColor,
        with_bicolored: bool,
    ) -> Vec<EdgeI> {
        self.get_incoming_sector_as_nb(vid, color, with_bicolored)
            .iter()
            .map(|nb| nb.edge)
            .collect()
    }

    pub fn get_incoming_sector(
        &self,
        vid: VertexI,
        color: SchnyderColor,
        with_bicolored: bool,
    ...
    pub fn outgoing_color(&self, nb: &NbVertex) -> Option<SchnyderColor> {
        self.map.try_edge(nb.edge).unwrap().color(match nb.end {
            Tail => Forward,
            Head => Backward,
        })
    }
    ...
    pub fn get_color_orientation(
        &self,
        e: EdgeI,
    ) -> GraphResult<Option<(SchnyderColor, VertexI, VertexI)>> {
        Ok(
            if let Unicolored(color, signum) = *self.map.edge_weight(e)? {
    ...
    pub fn is_inner_edge(&self, e: EdgeI) -> GraphResult<bool> {
        ...
        let weights = vec![
            self.map.vertex_weight(e.tail)?,
            self.map.vertex_weight(e.head)?,
        ];
        return Ok(!weights
            .iter()
            .any(|w| if let Suspension(_) = w { true } else { false }));
    }
    ...
    fn assemble_merge_data(&self, source: EdgeI, target: EdgeI) -> GraphResult<MergeData> {
        ...
        if let Ok((_hinge_vertex, nb1, nb2)) = knee {
            ...
            let (source_nb, target_nb) = if source == nb1.edge {
                (nb1, nb2)
            } else {
                (nb2, nb1)
            };
            ...
            let source_color = match self.outgoing_color(source_nb) {
                None => {
                    return GraphErr::new_err(
                        "Source edge for merge must be directed away from hinge",
                    )
                }
                Some(c) => c,
            };
            match self.incoming_color(target_nb) {
                None => {
                    return GraphErr::new_err(
                        "Target edge for merge must be directed towards hinge",
                    )
                }
                Some(c) => c,
            };
            ...
            Ok(MergeData {
                /*hinge_vertex, source_edge, source_nb,*/ source_color,
                /*target_edge,*/ target_nb, /*target_color*/
            })
        } else {
    ...
    pub fn merge(&mut self, source: EdgeI, target: EdgeI) -> GraphResult<Operation> {
        ...
        self.map
            .remove_embedded_edge_by_index(source, &(|a, _| a))?;

        Ok(Operation::merge(source_as_pair, target_as_pair))
    }

    fn assemble_split_data(
        &self,
        eid: EdgeI,
        hinge_vid: VertexI,
        ...
        if let Bicolored(tail_color, head_color) = e.weight {
            ...
            let target_vids = self
                .map
                .try_face(target_face)?
                .angles
                .iter()
            ...
            Ok(SplitData {
                target_vids,
                split_color,
                remaining_color: match hinge_end {
                    Tail => Unicolored(constant_color, Backward),
                    Head => Unicolored(constant_color, Forward),
                },
                target_face,
            })
        } else {
    ...
    pub fn split(
        ...
        self.map.add_embedded_edge(
            hinge_vid,
            effective_target,
            Unicolored(data.split_color, Forward),
            data.target_face,
        )?;
        ...
        Ok(Operation::split(
            hinge_vid,
            self.map.edge_opposite_vertex(eid, hinge_vid)?,
            effective_target,
        ))
    }
    ...
    pub fn merge_and_resplit(
        &mut self,
        source: EdgeI,
        target: EdgeI,
        resplit_target: Option<VertexI>,
    ) -> GraphResult<Vec<Operation>> {
        let merge_op = self.merge(source, target)?;
        let split_hinge = self
            .map
            .edge_opposite_vertex(target, merge_op.hinge_vertex)?;
    ...
    fn ext_splittable_(
        ...
        hinge: VertexI,
        opposite: VertexI,
    ) -> GraphResult<(VertexI, VertexI, SchnyderColor, SchnyderColor)> {
        ...
        if !angles.contains(&hinge) || !angles.contains(&opposite) {
            return GraphErr::new_err(
                "The hinges for an external split must lie on the outer face cycle.",
            );
        }

        {
            let between: Vec<_> = angles
                .cycle_by_element(&hinge, false)
                .skip(1)
                .take_while(|&&v| v != opposite)
                .collect();

            // there must not be a suspension vertex between the two hinges
            if between
                .iter()
                .any(|v| match self.map.vertex_weight(**v).unwrap() {
                    Suspension(_) => true,
                    _ => false,
                })
            {
                return GraphErr::new_err(
                    "The hinges for an must not enclose a suspension vertex.",
                );
            }
            ...
            if let (Some(&hinge_other), Some(&opposite_other)) = (between.first(), between.last()) {
                ...
                return if let (
                    Bicolored(hinge_color, hinge_color_bwd),
                    Bicolored(opposite_color, opposite_color_bwd),
                ) = (hinge_orientation, opposite_orientation)
                {
                    if hinge_color != opposite_color
                        || hinge_color_bwd != opposite_color
                        || opposite_color_bwd != hinge_color
                    {
                ...
                return GraphErr::new_err(
                    "The hinges for an external split must enclose more than one edge.",
                );
            }
        }
    }
    ...
    pub fn ext_split(&mut self, hinge: VertexI, opposite: VertexI) -> GraphResult<Operation> {
        let (hinge_other, opposite_other, hinge_color, opposite_color) =
            self.ext_splittable_(hinge, opposite)?;
        let new_edge = self.map.add_embedded_edge(
            hinge,
            opposite,
            Bicolored(hinge_color, opposite_color),
            self.outer_face,
        )?;

        let (e1, signum1) = self.map.edge_with_signum(hinge_other, hinge)?;
        self.map
            .set_edge_weight(e1, Unicolored(opposite_color, signum1))?;
        let (e2, signum2) = self.map.edge_with_signum(opposite_other, opposite)?;
        self.map
            .set_edge_weight(e2, Unicolored(hinge_color, signum2))?;
    ...
    fn ext_mergeable_(
        ...
        hinge: VertexI,
        opposite: VertexI,
    ) -> GraphResult<(SchnyderColor, SchnyderColor, VertexI, VertexI)> {
        if hinge == opposite {
        ...
        if !angles.contains(&hinge) || !angles.contains(&opposite) {
            return GraphErr::new_err(
                "The hinges for an external merge must lie on the outer face cycle.",
            );
        }
                ...
                if let Bicolored(fwd, bwd) = self.get_color(hinge, opposite)? {
                    ...
                    if let (
                        ...
                        return if hinge_color == bwd && opposite_color == fwd {
                            ...
                            GraphErr::new_err(
                                "Target edges do not have the right colors for an external merge.",
                            )
                        };
                    } else {
                        GraphErr::new_err(
                    ...
                    return GraphErr::new_err(
                        "Unidirected edges cannot be subject to an external merge.",
                    );
                }
            }
    ...
    pub fn get_admissible_ops(&self) -> GraphResult<Vec<Operation>> {
        ...
        for c in &[Red, Green, Blue] {
            ...
            let sector = outer_cycle
                .cycle_by_element(begin, false)
                .take_until(|v| *v == end)
                .collect_vec();
    ...
    pub fn revert_schnyder_contraction(
        &mut self,
        ...
        self.schnyder_uncontract(
            contraction.retained_vertex,
            contraction.color,
            Some(contraction.dropped_vertex),
            None,
        )
    }

    pub fn schnyder_uncontract(
        &mut self,
        pivot_vid: VertexI,
        color: SchnyderColor,
        ...
        //patch borders
        let border_ccw = self
            .find_outgoing_endvertex(pivot_vid, color.prev())
            .unwrap();
        let border_cw = self
            .find_outgoing_endvertex(pivot_vid, color.next())
            .unwrap();

        let (new_vid, _) = self
            .map
            .split_embedded_edge(
                // TODO maybe move the called function here
                eid,
                new_end,
    ...
    pub fn calculate_face_counts(&self) -> HashMap<VertexI, (usize, usize, usize)> {
        ...
        for &vid in self.map.vertex_indices() {
            if let Suspension(color) = self.map.vertex_weight(vid).unwrap() {
                ...
                let mut dual_no_cross_edges: HashSet<_> = primal_no_cross_edges
                    .iter()
                    .map(|eid| *edge_to_edge.get(eid).unwrap())
                    .collect();
                ...
                for color in SchnyderColor::all() {
                    ...
                    if f != self.outer_face {
                        ...
                        counts[color.index()] = dual
                            .connected_component(*start_vertex, &dual_no_cross_edges)
                            .expect(INVALID_WOOD)
                            .len();
                    } else {
    ...
    fn color_path(&self, vid: VertexI, color: SchnyderColor) -> GraphResult<Vec<VertexI>> {
        ...
        while let Normal(_) = self.map.vertex_weight(current_vertex)? {
            ...
            let out_index = self
                .find_outgoing(self.map.try_vertex(current_vertex)?, color)
                .unwrap();
    ...
    fn with_initialized_wood(mut self) -> GraphResult<Self> {
        ...
        let rgb = (
            suspension_vertices.iter().find(|&v| match v.weight {
                Suspension(Red) => true,
                _ => false,
            }),
            suspension_vertices.iter().find(|&v| match v.weight {
                Suspension(Green) => true,
                _ => false,
            }),
            suspension_vertices.iter().find(|&v| match v.weight {
                Suspension(Blue) => true,
                _ => false,
            }),
        );
        ...
        let (r, g, b) = match rgb {
            ...
            _ => return GraphErr::new_err(
                "There has to be one suspension vertex for each of the colors red, green, and blue",
            ),
        };
        ...
        if self.map.edges().any(|nb| match nb.weight {
            Bicolored(a, b) if a == b => true,
            _ => false,
        }) {
        ...
        for v in self.map.vertices() {
            ...
            match v.weight {
                Normal(_) => {
                    if state != begin_sector.prev() {
                        return GraphErr::new_err("Vertex rule violated");
                    }
                }
                Suspension(c) => {
                    if state != begin_sector.next() || last_outgoing != Some(c.prev()) {
                        return GraphErr::new_err("Vertex rule violated");
                    }
                }
            }
        ...
        for f in self.map.faces() {
            ...
            for (&v1, &v2) in f.angles.cycle(0, true).tuple_windows() {
                let eid = self.map.get_edge(v1, v2).map_err(|_| {
                    GraphErr::new(
                        "Face cycle rule cannot be checked as face cycle contains invalid edges",
                    )
                })?;
            ...
            if (fwd_colors.len() == 1 && !fwd_colors.iter().next().unwrap().is_none())
                || (bwd_colors.len() == 1 && !bwd_colors.iter().next().unwrap().is_none())
            {
...
impl PlanarMap<SchnyderVertexType, SchnyderEdgeDirection, ()> {
    fn find_outer_face(&self, r: VertexI, g: VertexI, b: VertexI) -> GraphResult<FaceI> {
        let outer_face_candidates = self
            .faces()
            .filter(|&f| f.angles.contains(&r) && f.angles.contains(&g) && f.angles.contains(&b))
            .collect_vec();
        ...
        let outer_face = outer_face_candidates
            .iter()
            .filter(|&&f| is_in_cyclic_order(&f.angles, &vec![r, g, b]))
            .collect_vec();
    ...
    pub fn get_incoming_by_color<'a>(
        &'a self,
        v: VertexI,
        color: SchnyderColor,
    ) -> Result<impl Iterator<Item = &'a VertexI> + 'a, IndexAccessError<VertexI>> {
        Ok(self
            .try_vertex(v)?
            .neighbors
            .iter()
...
impl TryFrom<PlanarMap<SchnyderVertexType, SchnyderEdgeDirection, ()>> for SchnyderMap {
    ...
    fn try_from(
        map: PlanarMap<SchnyderVertexType, SchnyderEdgeDirection, ()>,
    ) -> GraphResult<SchnyderMap> {
